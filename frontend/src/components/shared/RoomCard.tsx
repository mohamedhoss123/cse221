...
import { formatPrice, formatRoomTitle } from '#/lib/utils/formatters'
    ...
    <Card className="overflow-hidden border border-slate-200 rounded-2xl hover:shadow-lg transition-all duration-300 bg-white">
      <div className="relative h-56 overflow-hidden">
        {room.images && room.images.length > 0 ? (
          ...
          <div className="h-full w-full flex items-center justify-center bg-gradient-to-br from-[var(--expressive-background)] to-[var(--expressive-primary)]/20">
      ...
      <CardHeader className="pb-3">
        <CardTitle className="line-clamp-1 text-xl font-semibold text-[var(--expressive-primary)]">
          {room.name || formatRoomTitle(room.type)}
        </CardTitle>
      </CardHeader>
      <CardContent className="pb-4">
        <div className="flex items-center justify-between text-sm text-slate-600 mb-4">
          {room.capacity && (
            <div className="flex items-center gap-1.5 bg-slate-50 px-2.5 py-1 rounded-md border border-slate-100 text-slate-700">
